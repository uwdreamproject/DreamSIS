...
function loadForm(mentor) {
  $j('#indicator').addClass("visible");
  $j.get( mentor + "/onboarding_form", function( data ) {
...
function registerForm(id) {
  $j('#onboarding-form').submit( function() {
    $j('#indicator').addClass("visible");
    $j.ajax({
           type: "PUT",
           url: $j('#onboarding-form').attr('action'),
           data: $j("#onboarding-form").serialize(),
           success: function(data)
           {
               $j("#mentor-" + parseInt(id)).replaceWith(data);
           }
         }).always(function() {
           $j('#indicator').removeClass("visible");
         });
